...
- [Android] Fixed all custom view props arriving empty on React Native 0.87, which left Expo views without their content (e.g. blank `expo-image` images and invisible `expo-video` playback). React Native 0.87 moved the `Props::rawProps` initialization from the `Props` constructor into `ConcreteComponentDescriptor::cloneProps`, which the Expo view component descriptor overrides, so the props diff dispatched to Android view managers was always empty. ([#49503](https://github.com/expo/expo/pull/49503) by [@gabrieldonadel](https://github.com/gabrieldonadel))
